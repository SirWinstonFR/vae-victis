...
function renderZonePanel(zoneName) {
  ...
  const idees = (nations[zoneName] || {}).idees || [];
  const panelInner = document.getElementById('panel-inner');
  if (panelInner && !panelInner.querySelector('.idees-nationales-section')) {
    const TYPE_COLOR = { bonus: '#2a9a4a', malus: '#cc3030', neutre: '#3a7acc' };
    const P = 'polygon(50% 0%, 100% 38%, 82% 100%, 18% 100%, 0% 38%)';
    ...
    const slots = [0,1,2,3].map(function(i) {
      ...
      if (!id) {
        return '<div style="display:flex;flex-direction:column;align-items:center;gap:5px">'
          + '<div style="width:52px;height:52px;clip-path:' + P + ';background:#0d1828;display:flex;align-items:center;justify-content:center">'
          + '<div style="width:46px;height:46px;clip-path:' + P + ';background:#0a1422;display:flex;align-items:center;justify-content:center">'
      ...
      const detail = '<div style="display:none;grid-column:1/-1;padding:8px 10px;border-radius:var(--radius);border:1px solid ' + col + '33;background:' + col + '0a;font-size:11px;color:var(--c-text2);line-height:1.6">'
        ...
        + '</div>';
      const card = '<div style="display:flex;flex-direction:column;align-items:center;gap:5px;cursor:pointer"'
        + ' onclick="(function(el){var d=el.nextElementSibling;d.style.display=d.style.display===\'none\'?\'block\':\'none\'})(this)"'
        + ' title="' + id.nom + '">'
        + '<div style="width:52px;height:52px;clip-path:' + P + ';background:' + col + '44;display:flex;align-items:center;justify-content:center">'
        + '<div style="width:46px;height:46px;clip-path:' + P + ';background:#0a1422;overflow:hidden;display:flex;align-items:center;justify-content:center">'
        + icon + '</div></div>'
