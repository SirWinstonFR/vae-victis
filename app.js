...
function renderZonePanel(zoneName) {
  ...
  // Idées nationales — HTML construit avant setPanel (évite double insertion)
  const idees = (nation.idees || []);
  const _P = 'polygon(50% 0%, 100% 38%, 82% 100%, 18% 100%, 0% 38%)';
  const _TC = { bonus: '#2a9a4a', malus: '#cc3030', neutre: '#3a7acc' };
  const ideesSlots = [0,1,2,3].map(function(i) {
    const id  = idees[i];
    const col = id ? (_TC[id.type] || '#3a5a7a') : '#1a2e4a';
    if (!id) {
      return '<div style="display:flex;flex-direction:column;align-items:center;gap:5px">'
        + '<div style="width:52px;height:52px;clip-path:' + _P + ';background:#0d1828;display:flex;align-items:center;justify-content:center">'
        + '<div style="width:46px;height:46px;clip-path:' + _P + ';background:#0a1422;display:flex;align-items:center;justify-content:center">'
        + '<span style="font-size:16px;opacity:.2">?</span></div></div>'
        + '<div style="font-size:9px;color:#1a2e4a;font-family:Rajdhani,sans-serif;text-align:center">Vide</div></div>';
    }
    const icon = id.img
    ...
    const detail = '<div style="display:none;grid-column:1/-1;padding:8px 10px;border-radius:var(--radius);border:1px solid ' + col + '33;background:' + col + '0a;font-size:11px;color:var(--c-text2);line-height:1.6">'
    ...
    return '<div style="display:flex;flex-direction:column;align-items:center;gap:5px;cursor:pointer"'
      ...
      + ' title="' + id.nom + '">'
      + '<div style="width:52px;height:52px;clip-path:' + _P + ';background:' + col + '44;display:flex;align-items:center;justify-content:center">'
      + '<div style="width:46px;height:46px;clip-path:' + _P + ';background:#0a1422;overflow:hidden;display:flex;align-items:center;justify-content:center">'
      + icon + '</div></div>'
  ...
  const ideesHTML = '<div class="divider"></div>'
    + '<div class="sec"><i class="ti ti-star"></i> Id\u00e9es nationales</div>'
    + '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;padding:4px 2px">' + ideesSlots + '</div>';
